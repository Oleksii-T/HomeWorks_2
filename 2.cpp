...
The class will store individual coefficients (floating point numbers - double precision), moreover, public
interface will offer overloaded operators and methods to manipulate polynomials:
...
overloaded operator=
    the operator will copy the contents from one instance to another (if the automatically generated op= cannot be used
    with your implementation).
operator <<
    will output a textual form of the polynomial into an output stream. The output formatting must follow these rules:

        the polynomial will be displayed in a decreasing order of powers, i.e. from the highest power of x,terms with
        zero coefficient are not displayed in the output,
    ...
    The variable displayed in the output shall be x by default. There is manipulator, the manipulator may be used to
    change the default x to any other variable (bonus test only).
...
operator []
    is used to access (read / write) individual coefficients, the term is determined by the index
    (0 = absolute, 1 = x, 2 = x^2, ... ). The read form must be available even on const instances,
operator ()
    evaluates the value of the polynomial for the given value x (x is a double),
Degree()
    the method returns the degree of the polynomial (e.g. x^3+4 has degree of 3, 5 has degree 0,
    specifically 0 has degree 0).
manipulator polynomial_variable ( name )
    This manipulator changes the name of the variable use to display the polynomial. The default is to display the
    polynomial with variable x. The manipulator may be used, to change the variable name to any string (the parameter).
    The implementation of the manipulator is an extension - the manipulator is tested only in the bonus tests. If you do
    not intent to pass the bonus tests, keep the sample implementation of the manipulator. The delivered implementation
    of polynomial_variable does not do anything useful, it is just present, thus programs that use the manipulator
    actually compile.

Submit a source file with your implementation of CPolynomial class. The submitted file shall not contain any #include
directives nor main function. If your main function or #include remains in the file, please place them into a
conditional compile block.

This task does not provide the required class interface. Instead, you are expected to develop the interface yourself.
Use the description above, the attached examples, and your knowledge of overloaded operators.
Advice

    The testing environment uses output operator (<<) and indexing operator [] to examine your instance. If your overloaded
    operators << and [] do not work properly, the tests will be negative.
    Implement the output operator properly -- do not blindly sent the data to cout. Instead, send the data to the
    output stream passed as the parameter. Do not add any extra whitespace/newline characters.
    If your program does not compile (and especially if it compiles locally, however it does not compile in Progtest),
    there might be some problem in your interface design. Check your operator overloads, pay special attention to
    const qualifiers.
    You have to either implement your own functioning manipulator polynomial_variable, or the default (delivered)
    implementation must be preserved. If the default implementation is removed and your implementation is not supplied,
    the program will not compile.
    The bonus test requires very fast polynomial multiplication. The test multiplies polynomials of degree approx. 100000.
    Naive solution cannot pass the test, an efficient algorithms is required.
    CPolynomial class is tested for copying. Your implementation must include copy cons/op= if the implementation cannot
    use automatically generated copy cons/op=.
